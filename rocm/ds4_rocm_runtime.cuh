...
static const char *cuda_model_range_ptr(const void *model_map, uint64_t offset, uint64_t bytes, const char *what) {
        ...
        if (err == cudaSuccess) {
            ...
            if (err == cudaSuccess && reg_dev) {
                ...
                if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                    fprintf(stderr, DS4_GPU_LOG_PREFIX "mapped %s %.2f MiB\n",
                            what ? what : "weights",
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "cached %s %.2f MiB (total %.2f GiB)\n",
                what ? what : "weights",
                (double)bytes / 1048576.0,
...
struct ds4_rocm_runtime_config {
    ...
    int q8_prequant_batch;
    int disable_splitk_attn_out_low;
    int disable_shared_gate_up_fused_w32;
    int disable_shared_gate_up_pair;
    int disable_shared_gate_up_pair_swiglu;
    int attention_output_cublas;
...
static const ds4_rocm_runtime_config *cuda_runtime_config(void) {
    if (!g_rocm_cfg.initialized) {
        g_rocm_cfg.q8_prequant_decode = cuda_env_flag("DS4_CUDA_Q8_PREQUANT_DECODE");
        g_rocm_cfg.q8_prequant_batch = cuda_env_flag("DS4_CUDA_Q8_PREQUANT_BATCH");
        g_rocm_cfg.disable_splitk_attn_out_low = cuda_env_flag("DS4_CUDA_DISABLE_SPLITK_ATTN_OUT_LOW");
        g_rocm_cfg.disable_shared_gate_up_fused_w32 = cuda_env_flag("DS4_CUDA_DISABLE_SHARED_GATE_UP_FUSED_W32");
        g_rocm_cfg.disable_shared_gate_up_pair = cuda_env_flag("DS4_CUDA_DISABLE_SHARED_GATE_UP_PAIR");
        g_rocm_cfg.disable_shared_gate_up_pair_swiglu = cuda_env_flag("DS4_CUDA_DISABLE_SHARED_GATE_UP_PAIR_SWIGLU");
        g_rocm_cfg.attention_output_cublas = cuda_env_flag("DS4_CUDA_ATTENTION_OUTPUT_CUBLAS");
...
static int cuda_q8_f16_preload_allowed(const char *label, uint64_t in_dim, uint64_t out_dim) {
    if (cuda_q8_label_is_attention_output(label) &&
        ...
        !cuda_runtime_config()->attention_output_cublas &&
...
static const __half *cuda_q8_f16_ptr(
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "cached q8 fp16 %.2f MiB (total %.2f GiB)\n",
...
static const __half *cuda_q8_f16_transpose_ptr(
    ...
    g_q8_f16_bytes += out_bytes;
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "cached q8 fp16 transpose %.2f MiB (total %.2f GiB)\n",
                (double)out_bytes / 1048576.0,
                (double)g_q8_f16_bytes / 1073741824.0);
    }
    return dev;
...
static float *cuda_q8_f32_ptr(
    ...
    cudaError_t err = cudaMalloc(&dev, (size_t)out_bytes);
    if (err != cudaSuccess) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "q8 fp32 cache alloc failed (%.2f MiB): %s\n",
                (double)out_bytes / 1048576.0, cudaGetErrorString(err));
        (void)cudaGetLastError();
        return NULL;
    }
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "cached q8 fp32 %.2f MiB (total %.2f GiB)\n",
                (double)out_bytes / 1048576.0,
...
static int cuda_model_prefetch_range(const void *model_map, uint64_t model_size, uint64_t map_offset, uint64_t map_size) {
    ...
    if (err != cudaSuccess) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "model read-mostly advise skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
    err = cudaMemAdvise(pre_ptr, (size_t)pre_bytes, cudaMemAdviseSetPreferredLocation, loc);
    if (err != cudaSuccess) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "model preferred-location advise skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
    ...
    if (!g_model_prefetch_stream) {
        ...
        if (err != cudaSuccess) {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "model prefetch stream creation skipped: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    }
    ...
    if (err != cudaSuccess) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "model prefetch skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
    }
    if (getenv("DS4_CUDA_MODEL_PREFETCH_SYNC") != NULL) {
        ...
        if (err != cudaSuccess) {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "model prefetch sync failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    }
    ...
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "ATS/HMM prefetch queued %.2f GiB of model tensors in %.3fs\n",
    ...
    if (g_model_direct_fd >= 0 && g_model_direct_align > 1 && g_model_file_size != 0) {
        ...
        if (aligned_off <= g_model_file_size &&
            ...
            if (direct_errno == EINVAL || direct_errno == EFAULT || direct_errno == ENOTSUP || direct_errno == EOPNOTSUPP) {
                if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                    fprintf(stderr, DS4_GPU_LOG_PREFIX "direct model read disabled: %s\n", strerror(direct_errno));
                }
...
static char *cuda_model_arena_alloc(uint64_t bytes, const char *what) {
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        ...
        fprintf(stderr, DS4_GPU_LOG_PREFIX "model arena allocated %.2f MiB (arenas %.2f GiB)\n",
                (double)chunk / 1048576.0,
...
static const char *cuda_model_range_ptr_from_fd(
    ...
    if (g_model_range_bytes > limit || bytes > limit - g_model_range_bytes) {
        if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "direct %s %.2f MiB (cache budget %.2f GiB exhausted)\n",
                    what ? what : "weights",
                    (double)bytes / 1048576.0,
    ...
    if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "fd-cached %s %.2f MiB (total %.2f GiB)\n",
                what ? what : "weights",
                (double)bytes / 1048576.0,
                (double)g_model_range_bytes / 1073741824.0);
    }
...
static int cuda_model_copy_chunked(const void *model_map, uint64_t model_size, uint64_t map_offset, uint64_t map_size) {
    ...
    while (copied < map_size) {
        ...
        if (getenv("DS4_CUDA_MODEL_COPY_VERBOSE") != NULL && now - last_report >= 2.0) {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "model chunk copy %.2f/%.2f GiB\n",
...
extern "C" int ds4_gpu_end_commands(void) {
    if (getenv("DS4_CUDA_ASYNC_END_COMMANDS") != NULL ||
        getenv("DS4_HIP_ASYNC_END_COMMANDS") != NULL) {
...
extern "C" int ds4_gpu_set_model_map(const void *model_map, uint64_t model_size) {
    ...
    if (copy_env && copy_env[0]) {
        ...
        if (err == cudaSuccess) {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "legacy-copying %.2f GiB model to device memory\n",
                    (double)model_size / 1073741824.0);
            ...
            if (err == cudaSuccess) {
                ...
                fprintf(stderr, DS4_GPU_LOG_PREFIX "legacy model copy complete in %.3fs\n", t1 - t0);
                return 1;
            }
            fprintf(stderr, DS4_GPU_LOG_PREFIX "legacy model copy failed: %s\n", cudaGetErrorString(err));
            (void)cudaFree(dev);
            (void)cudaGetLastError();
        } else {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "model allocation skipped: %s\n", cudaGetErrorString(err));
    ...
    if (err == cudaSuccess) {
        ...
        if (err == cudaSuccess && dev) {
            ...
            fprintf(stderr, DS4_GPU_LOG_PREFIX "registered %.2f GiB model mapping for device access\n",
                    (double)model_size / 1073741824.0);
        } else {
            fprintf(stderr, DS4_GPU_LOG_PREFIX "host registration pointer lookup failed: %s\n", cudaGetErrorString(err));
            (void)cudaGetLastError();
        }
    } else {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "host registration skipped: %s\n", cudaGetErrorString(err));
        (void)cudaGetLastError();
    }
    return 1;
}
    ...
    if (fd >= 0) {
        ...
        if (getenv("DS4_CUDA_NO_DIRECT_IO") == NULL) {
            ...
            if (direct_fd >= 0) {
                ...
                if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                    fprintf(stderr, DS4_GPU_LOG_PREFIX "model direct I/O enabled (align=%llu)\n",
                            (unsigned long long)g_model_direct_align);
                }
            } else if (getenv("DS4_CUDA_WEIGHT_CACHE_VERBOSE")) {
                fprintf(stderr, DS4_GPU_LOG_PREFIX "model direct I/O unavailable: %s\n", strerror(errno));
            }
        }
#endif
    }
