...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    if (full) {
        ...
        if (tool == DS4_HELP_DS4 || tool == DS4_HELP_AGENT || tool == DS4_HELP_SERVER) {
            ...
            opt(fp, c, "--dspark-confidence F", "Enable DSpark with confidence pruning threshold 0..1. Greedy/opportunistic default: Metal 0.6, CUDA/ROCm 0.7 (DeepSeek V4.1 greedy on ROCm: 0.5); exact sampling: 0.8");
